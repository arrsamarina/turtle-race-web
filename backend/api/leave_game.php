...
try {
    ...
    // Вызов SQL функции leave_game (обёртка над leave_active_game и leave_lobby)
    $stmt = $db->prepare('SELECT leave_game(:p_token, :p_game_id)::json AS data');
    $stmt->execute([
        ':p_token'   => $token,
        ':p_game_id' => $game_id,
    ]);
    ...
    if ($row && isset($row['data'])) {
        ...
        if ($result && isset($result['status'])) {
            if ($result['status'] === 'ok') {
                // Успешный выход из игры/лобби - возвращаем JSON как есть
                // Функция leave_game автоматически вызывает leave_active_game или leave_lobby
                echo $row['data'];
            } else {
                // Ошибка выхода из игры/лобби
                ...
                if ($errorCode === 'invalid_token') {
                    ...
                    $httpCode = 404;
                } else if ($errorCode === 'game_not_started_use_leave_lobby') {
                    $httpCode = 400;
